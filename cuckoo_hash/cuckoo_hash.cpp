...
void cuckoo::m_rehash(const std::size_t new_sz)
{
...
std::experimental::optional<cuckoo::data_t> cuckoo::min() const
{
    if (!m_size)
...
std::experimental::optional<cuckoo::data_t> cuckoo::max() const
{
    if (!m_size)
        return std::experimental::nullopt;
...
std::ostream& dsa::operator<<(std::ostream& os, const dsa::cuckoo& obj)
{
